...
def course_about(request, course_id):  # pylint: disable=too-many-statements
    ...
    return redirect(reverse('about_course_with_slug', args=[course_manage.slug]))
...
def course_about_with_slug(request, slug_id):
    ...
    Display the course's about page.
    """
    course_manage = CourseManage.objects.get(slug=slug_id)

    course_key = course_manage.course.id
    ...
    with modulestore().bulk_operations(course_key):
        ...
        if _is_shopping_cart_enabled:
            ...
            login_then_add_to_cart_link = "{reg_url}?course_id={course_id}&enrollment_action=add_to_cart".format(
                reg_url=reverse('signin_user'), course_id=six.moves.urllib.parse.quote(str(course_id))
            )
